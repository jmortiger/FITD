...
void GereFrappe(void)
{
	switch (currentProcessedActorPtr->animActionType) {
		...
		case 5: // DO_TIR
		{
			...
			// If no one has been touched...
			if (touchedActor == -1) {
				// ...The projectile hit the wall & didn't damage anything.
				InitSpecialObjet(2, animMoveX, animMoveY, animMoveZ, currentProcessedActorPtr->stage, currentProcessedActorPtr->room, 0, -currentProcessedActorPtr->beta, 0, NULL); // TODO: This is the same on both branches; why?

				currentProcessedActorPtr->animActionType = 0;
			} else { // ...otherwise...
				// ...the projectile should damage what got hit.
				InitSpecialObjet(2, animMoveX, animMoveY, animMoveZ, currentProcessedActorPtr->stage, currentProcessedActorPtr->room, 0, -currentProcessedActorPtr->beta, 0, NULL); // TODO: This is the same on both branches; why?
